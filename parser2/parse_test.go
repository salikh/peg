...
var syntaxParseTests = []syntaxParseTest{
	{`A <- .`, `
		(Grammar
	   (Rule
...
func TestSyntaxParse(t *testing.T) {
	for _, tt := range syntaxParseTests {
		t.Run(tt.source, func(t *testing.T) {
			t.Logf("grammar source:\n%s\n---", tt.source)
			result, err := parse(tt.source)
			if err != nil {
				t.Errorf("parse(%s) returned error %s, want success",
					tt.source, err)
				return
			}
			t.Logf("grammar tree:\n%s\n---", result.Tree)
			...
			diffs := tree.Diff(result.Tree, wantTree)
			if len(diffs) > 0 {
				t.Errorf("Parse(%s) returned tree \n%s\n---, want \n%s\n---\ndiff:\n%s",
					tt.source, result.Tree, wantTree, strings.Join(diffs, "\n"))
			}
		})
	}
}

type semanticParseTest struct {
	// source is the PEG grammar source.
	source string
...
var semanticParseTests = []semanticParseTest{
	{`A <- .`,
		`(Grammar
	...
	{`A <- [[:any:]]`, // Custom extension.
		`(Grammar
	    (Rule text("A") (RHS (Choice (Term :CharClass("[:any:]"))))))`},
	{`A <- "abc"`,
		`(Grammar
	    (Rule text("A") (RHS (Choice (Term :Literal("abc"))))))`},
	{`A <- 'singlequote-newline\n'`,
		`(Grammar
	    (Rule text("A") (RHS (Choice (Term :Literal("singlequote-newline\\n"))))))`},
	{`A <- "doublequote-newline\n"`,
		`(Grammar
	    (Rule text("A") (RHS (Choice (Term :Literal("doublequote-newline\n"))))))`},
	{`A <-
	'abc'`,
		`(Grammar
	    (Rule text("A") (RHS (Choice (Term :Literal("abc"))))))`},
...
func TestSemantic(t *testing.T) {
	for _, tt := range semanticParseTests {
		t.Run(tt.source, func(t *testing.T) {
			t.Logf("grammar source:\n%s\n---", tt.source)
			result, err := parse(tt.source)
			if err != nil {
			...
			if err != nil {
				t.Errorf("convert(%s) returned error %s, want success", tt.source, err)
				return
			}
			t.Logf("grammar:\n%s\n---", grammar)
			got := grammar.String()
			gotTree, err := tree.Parse(got)
			if err != nil {
			...
			diffs := tree.Diff(gotTree, wantTree)
			if len(diffs) > 0 {
				t.Errorf("Parse(%s) returned tree \n%s\n---, want \n%s\n---\ndiffs:\n%s",
					tt.source, gotTree, wantTree, strings.Join(diffs, "\n"))
			}
		})
	}
}
...
func TestParseError(t *testing.T) {
	for _, tt := range invalidParseTests {
		t.Run(tt.source, func(t *testing.T) {
			t.Logf("grammar source:\n%s\n---", tt.source)
			result, sErr := parse(tt.source)
			if sErr == nil && tt.syntaxErr != "" {
				t.Errorf("parse(%s) returned success, want error [%s]",
			...
			if sErr != nil {
				...
				if !re.MatchString(sErr.Error()) {
					t.Errorf("parse(%s) returned error [%s], but want error [%s]",
						tt.source, sErr.Error(), tt.syntaxErr)
				}
				return
			}
			_, sErr = convert(result.Tree)
			// Expect the error during
			if sErr == nil && tt.semanticErr != "" {
				t.Errorf("convert(%s) returned success, want error [%s]",
			...
			if sErr != nil {
				...
				if !re.MatchString(sErr.Error()) {
					t.Errorf("convert(%s) returned error [%s], but want error [%s]",
