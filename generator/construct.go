...
func (ca *accessor) GetString(name string) (string, error) {
	...
	if !ok {
		return "", fmt.Errorf("expected %s as string, got none", name)
	}
	s, ok := val.(string)
	if !ok {
		return "", fmt.Errorf("expected %s as string, got %s",
...
func (ca *accessor) GetTyped(name string, ty interface{}) (interface{}, error) {
	...
	val, ok := ca.children[name]
	if !ok {
		return nil, fmt.Errorf("expected %s as %s, got none", name, reflect.TypeOf(ty))
	}
	...
	return nil, fmt.Errorf("expected %s, got %s", reflect.TypeOf(ty), reflect.TypeOf(val))
}
		...
		if !ok {
			...
			if reflect.TypeOf(have) == reflect.TypeOf(val) {
				...
				return nil, fmt.Errorf("incompatible types, have %s and got %s",
					reflect.TypeOf(have), reflect.TypeOf(val))
			}
		}
	}
	val, err := callback(n.Label, ca)
	if err != nil {
		return nil, fmt.Errorf("error constructing %s: %s", n.Label, err)
	}
	err = ca.Check()
	if err != nil {
		return nil, fmt.Errorf("error constructing %s: %s", n.Label, err)
